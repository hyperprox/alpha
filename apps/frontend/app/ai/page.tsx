        ...
        <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
          {!model.installed ? (
            ...
            <button onClick={() => onDelete(model.id)}
              style={{ padding: '5px 10px', borderRadius: 6, fontSize: 11, cursor: 'pointer', ...mono, background: 'transparent', color: '#374151', border: '1px solid transparent' }}
...
function ExecutionView({ job, onDone }: { job: JobProgress; onDone: () => void }) {
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto' }}>
      ...
      <div style={{
        background: CARD,
        border: `1px solid ${job.status === 'failed' ? '#ef444430' : job.status === 'completed' ? '#22c55e30' : `${ACCENT}30`}`,
        borderRadius: 12,
        padding: '1.25rem',
        marginBottom: '1rem',
      }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: 14, fontWeight: 700, color: job.status === 'failed' ? '#ef4444' : job.status === 'completed' ? '#22c55e' : ACCENT, ...mono, marginBottom: 4 }}>
          ...
          {isComplete && (
            <button onClick={onDone}
              style={{ padding: '6px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer', ...mono, background: `${ACCENT}15`, color: ACCENT, border: `1px solid ${ACCENT}30` }}>
              New deployment →
            </button>
          )}
        </div>
        ...
        {/* Progress bar */}
        {!isComplete && (
          <div style={{ marginTop: '1rem', height: 3, background: '#111827', borderRadius: 2, overflow: 'hidden' }}>
            <div style={{
              height: '100%',
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: step.output || step.error || step.commands ? 6 : 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: step.status === 'pending' ? '#4b5563' : '#e2e8f0', ...mono }}>
                      {i + 1}. {step.label}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      {duration && <span style={{ fontSize: 10, color: '#4b5563', ...mono }}>{duration}</span>}
                  ...
                  {/* Output */}
                  {step.output && (
                    <div style={{ fontSize: 11, color: '#22c55e', ...mono, background: '#060a10', padding: '6px 10px', borderRadius: 6, marginBottom: step.commands ? 8 : 0 }}>
                      {step.output}
                    </div>
                  )}
                  ...
                  {/* Error */}
                  {step.error && (
                    <div style={{ fontSize: 11, color: '#ef4444', ...mono, background: 'rgba(239,68,68,0.08)', padding: '6px 10px', borderRadius: 6, border: '1px solid rgba(239,68,68,0.2)' }}>
                      ✗ {step.error}
                    </div>
                  )}
                  ...
                  {step.commands && step.commands.length > 0 && (
                    <div style={{ marginTop: 6 }}>
                      <div style={{ fontSize: 10, color: '#f59e0b', ...mono, marginBottom: 4 }}>
                        ⚙ Run these commands in CT {job.lxcVmid ?? '?'} console:
                      </div>
                      <div style={{ background: '#060a10', border: '1px solid #1a2035', borderRadius: 6, padding: '8px 10px', overflow: 'auto', maxHeight: 160 }}>
                        {step.commands.map((cmd, ci) => (
                          <div key={ci} style={{ fontSize: 11, color: cmd.startsWith('#') ? '#4b5563' : '#e2e8f0', ...mono, lineHeight: 1.6 }}>
                            {cmd}
                          </div>
                        ))}
                      </div>
      ...
      {job.status === 'failed' && job.steps.every(s => s.status !== 'failed') && (
        <div style={{ marginTop: 12, padding: '10px 14px', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 8, fontSize: 12, color: '#ef4444', ...mono }}>
...
export default function AIPage() {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: BG }}>
      <div style={{ fontSize: 12, color: '#374151', ...mono }}>initializing ai...</div>
    </div>
  )
        ...
        {(['wizard', 'models', 'settings'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            style={{
              ...
              background: 'transparent', color: tab === t ? ACCENT : '#4b5563',
          ...
          {liveJob ? (
            ...
            <div style={{ maxWidth: 760, margin: '0 auto' }}>
              ...
              <div ref={chatRef} style={{ minHeight: 200, maxHeight: 360, overflowY: 'auto', marginBottom: '1rem', display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxWidth: 320, margin: '0 auto' }}>
                      {[
                        ...
                        <button key={ex} onClick={() => setPrompt(ex.replace(/"/g, ''))}
                          style={{ padding: '6px 12px', borderRadius: 6, fontSize: 11, cursor: 'pointer', ...mono, background: '#0d1220', color: '#6b7280', border: `1px solid ${BORDER}`, textAlign: 'left', transition: 'border-color .15s' }}
                ...
                {messages.map((msg, i) => (
                  <div key={i} style={{ alignSelf: msg.role === 'user' ? 'flex-end' : 'flex-start', maxWidth: '80%', padding: '10px 14px', borderRadius: 10, fontSize: 13, lineHeight: 1.5, background: msg.role === 'user' ? `${ACCENT}15` : CARD, color: msg.role === 'user' ? ACCENT : '#e2e8f0', border: `1px solid ${msg.role === 'user' ? ACCENT + '30' : BORDER}`, ...mono }}>{msg.content}</div>
                ))}
                {planning && (
                  <div style={{ alignSelf: 'flex-start', padding: '10px 14px', borderRadius: 10, background: CARD, border: `1px solid ${BORDER}`, fontSize: 12, color: '#6b7280', ...mono }}>thinking...</div>
                )}
              </div>
              {plan && (
                <div style={{ background: CARD, border: `1px solid ${ACCENT}30`, borderRadius: 12, padding: '1.25rem', marginBottom: '1rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                    <button onClick={() => setPlan(null)} style={{ padding: '8px 16px', borderRadius: 8, fontSize: 12, cursor: 'pointer', ...mono, background: 'transparent', color: '#6b7280', border: `1px solid ${BORDER}` }}>Cancel</button>
                    <button onClick={() => { if (window.confirm(`Execute ${plan?.steps?.length} steps to deploy ${plan?.service}?`)) handleExecute() }} disabled={executing}
                      style={{ padding: '8px 20px', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: executing ? 'default' : 'pointer', ...mono, background: executing ? `${ACCENT}20` : `${ACCENT}15`, color: executing ? `${ACCENT}60` : ACCENT, border: `1px solid ${ACCENT}30` }}>
        ...
        <div>
          {catalog?.hardware && (
            <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: '1rem', marginBottom: '1.5rem', display: 'flex', gap: '2rem' }}>
              <div><div style={{ fontSize: 10, color: '#4b5563', ...mono, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 4 }}>Cluster RAM</div><div style={{ fontSize: 20, fontWeight: 700, color: '#e2e8f0', ...mono }}>{catalog.hardware.totalRamGB.toFixed(0)}GB</div></div>
              <div><div style={{ fontSize: 10, color: '#4b5563', ...mono, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 4 }}>GPU VRAM</div><div style={{ fontSize: 20, fontWeight: 700, color: catalog.hardware.maxVramGB > 0 ? ACCENT : '#374151', ...mono }}>{catalog.hardware.maxVramGB > 0 ? `${catalog.hardware.maxVramGB.toFixed(0)}GB` : 'none'}</div></div>
              <div><div style={{ fontSize: 10, color: '#4b5563', ...mono, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 4 }}>CPU Cores</div><div style={{ fontSize: 20, fontWeight: 700, color: '#e2e8f0', ...mono }}>{catalog.hardware.totalCores}</div></div>
              <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center' }}><div style={{ fontSize: 12, color: '#6b7280', maxWidth: 240 }}>Models marked <span style={{ color: ACCENT }}>⚡ recommended</span> are best for your hardware.</div></div>
            </div>
          )}
      ...
      {tab === 'settings' && (
        <div style={{ maxWidth: 560 }}>
          ...
          <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: '1.25rem', marginBottom: '1rem' }}>
            ...
            <div style={{ fontSize: 12, color: '#6b7280', marginBottom: '1rem' }}>Already running Ollama on another machine? Point HyperProx at it.</div>
            <div style={{ marginBottom: 8 }}>
              ...
              {detected.length > 0 && (<div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>{detected.map(d => (<button key={d.url} onClick={() => setOllamaUrl(d.url)} style={{ padding: '6px 10px', borderRadius: 6, fontSize: 11, cursor: 'pointer', textAlign: 'left', ...mono, background: ollamaUrl === d.url ? `${ACCENT}15` : '#0a0f1a', color: ollamaUrl === d.url ? ACCENT : '#6b7280', border: `1px solid ${ollamaUrl === d.url ? ACCENT + '30' : BORDER}` }}>✓ {d.name} — {d.url}</button>))}</div>)}
              {!detecting && detected.length === 0 && (<span style={{ marginLeft: 10, fontSize: 11, color: '#374151', ...mono }}>or enter manually below</span>)}
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <input value={ollamaUrl} onChange={e => setOllamaUrl(e.target.value)} placeholder="http://your-server:11434"
                style={{ flex: 1, padding: '8px 12px', borderRadius: 8, background: '#080c14', border: `1px solid ${BORDER}`, color: '#e2e8f0', fontSize: 13, outline: 'none', ...mono }} />
              <button onClick={handleConnect} disabled={!ollamaUrl || connecting}
                style={{ padding: '8px 16px', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: (!ollamaUrl || connecting) ? 'default' : 'pointer', ...mono, background: `${ACCENT}15`, color: ACCENT, border: `1px solid ${ACCENT}30`, opacity: (!ollamaUrl || connecting) ? 0.5 : 1 }}>
                {connecting ? 'connecting...' : 'Connect'}
              </button>
            </div>
            {connectErr && (<div style={{ marginTop: 8, fontSize: 12, color: '#ef4444', padding: '8px 10px', background: 'rgba(239,68,68,0.08)', borderRadius: 6, border: '1px solid rgba(239,68,68,0.2)', ...mono }}>✗ {connectErr}</div>)}
            {status?.connected && (
              <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ fontSize: 12, color: '#22c55e', ...mono }}>✓ Connected to {status.url} — {status.models.length} model{status.models.length !== 1 ? 's' : ''} installed</div>
                <button onClick={async () => { await fetch('/api/ai/disconnect', { method: 'DELETE' }); await fetchAll() }} style={{ fontSize: 11, padding: '4px 10px', borderRadius: 6, cursor: 'pointer', ...mono, background: 'transparent', color: '#ef4444', border: '1px solid rgba(239,68,68,0.3)' }}>disconnect</button>
              </div>
            )}
          </div>
          <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: '1.25rem', opacity: 0.6 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: '#6b7280' }}>HyperProx creates a dedicated Ollama LXC on your cluster automatically.</div>
              </div>
